      ...
      <v-col cols="12" md="8">
        <v-card rounded="xl">
          <v-toolbar color="transparent">
            <v-toolbar-title>Artizan Bookings</v-toolbar-title>
          </v-toolbar>

          <v-card-text>
            <v-table class="rounded-xl" v-if="jobs.length >= 1">
              <thead>
                <tr>
              ...
              <tbody>
                <v-hover v-for="job in jobs?.slice(0, 50)" :key="job.id">
                  <template v-slot:default="{ isHovering, props }">
                    <tr @click="setActiveDialog(job)" v-bind="props"
                      :class="isHovering ? 'bg-indigo-lighten-5' : undefined" style="cursor: pointer;">
                      <td class="text-grey-darken-2 text-caption font-weight-bold">{{ job?.time }}</td>
                      <td class="text-grey-darken-2 text-caption font-weight-bold">{{ job?.date }}</td>
                      <td class="text-grey-darken-2 text-caption font-weight-bold">{{
                        (job?.budget).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</td>
                      <td>
                        <v-chip
                          :color="job?.status === 'pending' ? 'amber' : job?.status === 'approved' ? 'indigo' : job?.status === 'declined' ? 'red' : 'green'">
                          <span class="font-weight-bold text-caption">{{ job?.status }}</span>
                        </v-chip>
                      </td>
                    </tr>
                  </template>
            ...
            <v-table class="rounded-xl" v-if="history.length >= 1">
              ...
              <tbody>
                <v-hover v-for="history in history?.slice(0, 50)" :key="history.id">
                  <template v-slot:default="{ isHovering, props }">
                    <tr @click="setActiveDialog(history)" v-bind="props"
                      :class="isHovering ? 'bg-indigo-lighten-5' : undefined" style="cursor: pointer;">
              ...
              <v-card flat>
                ...
                <v-card-text class="d-flex justify-space-between">
                  ...
                  <v-chip size="small"
                    ...
                    <span class="font-weight-bold"
                      :class="activeDialog?.status == 'pending' ? 'text-amber-darken-2' : activeDialog?.status == 'approved' ? 'text-indigo-darken-2' : activeDialog?.status == 'declined' ? 'text-red-darken-2' : ''">{{
                        (activeDialog?.budget).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</span>
                  </v-chip>
                </v-card-text>
                ...
                <v-card-text class="text-body-1 text-grey-darken-3">{{ activeDialog?.description }}</v-card-text>
              </v-card>
            </v-col>
            <v-col cols="12" md="5">
...
export default {
  ...
  methods: {
    ...
    async getBookings() {
      const exsistingJobs = (await getDoc(doc(db, 'artizans', this.$route.params.id))).data()

      const acceptedBookings = exsistingJobs.acceptedBookings

      if (acceptedBookings == undefined) return

      acceptedBookings.forEach(data => {
